...
function Logger.setDebug(debug)
	debug = dcsex.math.toBoolean(debug)
...
function Logger.setDefaultLogLevel(lvl)
	lvl = dcsex.check.tblkey(lvl, Logger.level, "Logger.level")
	_settings.level = lvl
end
...
function Logger.setAllLogLevels(lvltbl)
	lvltbl = dcsex.check.table(lvltbl)
...
function Logger.setPrefix(prefix)
	prefix = dcsex.check.string(prefix)
...
function Logger:__init(name)
	self.name   = dcsex.check.string(name)
