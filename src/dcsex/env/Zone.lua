...
function Zone:__init(zonetbl)
	...
	self.point   = dcsex.vector.Vec2(zonetbl)
	...
	if zonetbl.verticies ~= nil then
		...
		for _, v in ipairs(zonetbl.verticies) do
			table.insert(self.verticies, dcsex.vector.Vec2(v))
		end
	end
...
-- @return dcsex.vector.Vec2
function Zone:getPoint()
	local point = self.point or dcsex.vector.Vec2()
...
function Zone:getPropertyBoolean(name)
	return dcsex.math.toBoolean(self:getProperty(name))
end
...
function Zone:getPropertyFloat(name, min, max)
	...
	return dcsex.math.clamp(val, min, max)
end
