...
function Waypoint:__init(point, wtype, action, speed, name)
	local vec3 = dcsex.vector.Vec3(point)
...
function Waypoint.createGround(point, speed, formation, name)
	local pt = dcsex.vector.Vec3(point)
	pt.y = land.getHeight(dcsex.vector.Vec2(point):get())

	local wpt = Waypoint(point, Waypoint.wpType.TURNING_POINT,
			dcsex.check.tblkey(formation or
...
function Waypoint.createLandingTakeoff(airbase, wtype, speed)
	dcsex.check.table(airbase)
	local point = dcsex.vector.Vec3(airbase:getPoint())
...
function Waypoint:setPoint(vec2, wptype, action)
	self.point = dcsex.vector.Vec2(dcsex.check.table(vec2))
	self.type = dcsex.check.tblkey(wptype or
				       Waypoint.wpType.TURNING_POINT,
				       Waypoint.wpType,
				       "Waypoint.wpType")
...
function Waypoint:setAlt(alt, alttype)
	self.alt      = dcsex.check.number(alt)
	self.alt_type = dcsex.check.tblkey(alttype or
...
function Waypoint:setSpeed(spd)
	self.speed        = dcsex.check.number(spd)
...
function Waypoint:setETA(time)
	self.ETA          = dcsex.check.number(time)
...
-- @param tasktype dcsex.enum.TASKTYPE
...
function Waypoint:addTask(task, tasktype, idx)
	local tbl = dcsex.ai.exec.wrapTask(task, tasktype)
...
function Waypoint:get()
	...
	tbl = dcsex.table.merge(tbl, self.point:get())
	if next(self.tasks) then
		local t = {}

		for _, task in ipairs(self.tasks) do
			local data = task.data
			if task.type == dcsex.enum.TASKTYPE.OPTION then
				data = dcsex.ai.options.wrappedOption(
					task.data)
			elseif task.type == dcsex.enum.TASKTYPE.COMMAND then
				data = dcsex.ai.commands.wrappedCommand(
					task.data)
			end
			table.insert(t, data)
		end

		tbl.task = dcsex.ai.tasks.combo(t)
	end
	return tbl
end
