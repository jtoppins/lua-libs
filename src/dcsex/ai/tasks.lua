...
local vector = require("dcsex.vector")
...
local optionalParams = {
	...
	["groupAttack"] = function(params, op)
		params.groupAttack = dcsex.math.toBoolean(op.groupAttack)
	end,
	["attackType"]  = function(params, op)
		params.attackType = check.tblkey(op.attackType,
						 enum.ATTACKTYPE,
						 "dcsex.enum.ATTACKTYPE")
	end,
...
function _t.facAttackGroup(group, wpnType, designation, datalink,
	...
	if datalink ~= nil then
		task.params.datalink = dcsex.math.toBoolean(datalink)
	end
...
function _t.orbit(pat, options)
	local params = {}
	params.pattern  = check.tblkey(pat, dcsex.enum.ORBITPATTERN,
					"dcsex.enum.ORBITPATTERN")
	...
	if params.pattern == enum.ORBITPATTERN.ANCHORED then
		...
		if options.clockWise ~= nil then
			params.clockWise =
				dcsex.math.toBoolean(options.clockWise)
		end
	end
